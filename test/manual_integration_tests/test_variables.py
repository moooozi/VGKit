...
if TEST_CONFIGURE:
    entry_1.configure(textvariable=txt_var)
if TEST_REMOVING:
    entry_1.configure(textvariable="")
# entry_1.delete(0, "end")
# entry_1.insert(0, "sadsad")

label_1 = vgk.Label(app, width=200, textvariable=txt_var)
label_1.pack(pady=15)
if TEST_CONFIGURE:
    label_1.configure(textvariable=txt_var)
if TEST_REMOVING:
    label_1.configure(textvariable="")

button_1 = vgk.Button(app, width=200, textvariable=txt_var)
button_1.pack(pady=15)
int_var = tkinter.IntVar(value=10)
if TEST_CONFIGURE:
    button_1.configure(textvariable=int_var)
if TEST_REMOVING:
    button_1.configure(textvariable="")

slider_1 = vgk.Slider(app, width=200, from_=0, to=3, variable=int_var)
slider_1.pack(pady=15)
if TEST_CONFIGURE:
    slider_1.configure(variable=int_var)
if TEST_REMOVING:
    slider_1.configure(variable="")
int_var.set(2)

slider_2 = vgk.Slider(app, width=200, from_=0, to=3, variable=int_var)
slider_2.pack(pady=15)
if TEST_CONFIGURE:
    slider_2.configure(variable=int_var)
if TEST_REMOVING:
    slider_2.configure(variable="")

label_2 = vgk.Label(app, width=200, textvariable=int_var)
label_2.pack(pady=15)

progress_1 = vgk.ProgressBar(app, width=200, variable=int_var)
progress_1.pack(pady=15)
if TEST_CONFIGURE:
    progress_1.configure(variable=int_var)
if TEST_REMOVING:
    progress_1.configure(variable="")

check_var = tkinter.StringVar(value="on")
check_1 = vgk.CheckBox(
    app,
    text="check 1",
    variable=check_var,
    onvalue="on",
...
if TEST_CONFIGURE:
    check_1.configure(variable=check_var)
if TEST_REMOVING:
    check_1.configure(variable="")
...
if TEST_REMOVING:
    check_2.configure(variable="")

label_3 = vgk.Label(app, width=200, textvariable=check_var)
...
optionmenu_1 = vgk.OptionMenu(
    master=app, variable=None, values=["Option 1", "Option 2", "Option 3"]
)
optionmenu_1.pack(pady=20, padx=10)
optionmenu_1.configure(variable=optionmenu_var)
combobox_1 = vgk.ComboBox(master=app, values=["Option 1", "Option 2", "Option 3"])
combobox_1.pack(pady=20, padx=10)
combobox_1.configure(variable=optionmenu_var)

radio_1 = vgk.RadioButton(app, textvariable=txt_var)

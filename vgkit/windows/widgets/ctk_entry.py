...
class CTkEntry(CTkBaseClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._placeholder_text_color = (
            ...
            else self._check_color_type(placeholder_text_color)
        )
        self._border_color = (
        ...
        # shape
        self._corner_radius = (
            ThemeManager.theme["CTkEntry"]["corner_radius"]
            if corner_radius is None
            else corner_radius
        )
        self._border_width = (
            ThemeManager.theme["CTkEntry"]["border_width"] if border_width is None else border_width
        )
        ...
        if not (self._textvariable is None or self._textvariable == ""):
            self._textvariable_callback_name = self._textvariable.trace_add(
                "write", self._textvariable_callback
            )
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        if "placeholder_text_color" in kwargs:
            self._placeholder_text_color = self._check_color_type(
                kwargs.pop("placeholder_text_color")
            )
            require_redraw = True
        ...
        if "show" in kwargs:
            if self._placeholder_text_active:
                self._pre_placeholder_arguments["show"] = kwargs.pop(
                    "show"
                )  # remember show argument for when placeholder gets deactivated
            else:
                self._entry.configure(show=kwargs.pop("show"))

        self._entry.configure(
            **pop_from_dict_by_set(kwargs, self._valid_tk_entry_attributes)
        )  # configure Tkinter.Entry

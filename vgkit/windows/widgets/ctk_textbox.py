...
class CTkTextbox(CTkBaseClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._create_grid_for_text_and_scrollbars(
            re_grid_textbox=True, re_grid_x_scrollbar=True, re_grid_y_scrollbar=True
        )
    ...
    def _create_grid_for_text_and_scrollbars(
        self, re_grid_textbox=False, re_grid_x_scrollbar=False, re_grid_y_scrollbar=False
    ):
    ...
    def _check_if_scrollbars_needed(self, event=None):
        ...
        if self._scrollbars_activated:
            if (
                self._textbox.xview() != (0.0, 1.0) and not self._x_scrollbar.winfo_ismapped()
            ):  # x scrollbar needed
                self._hide_x_scrollbar = False
                self._create_grid_for_text_and_scrollbars(re_grid_x_scrollbar=True)
            elif (
                self._textbox.xview() == (0.0, 1.0) and self._x_scrollbar.winfo_ismapped()
            ):  # x scrollbar not needed
                self._hide_x_scrollbar = True
                self._create_grid_for_text_and_scrollbars(re_grid_x_scrollbar=True)

            if (
                self._textbox.yview() != (0.0, 1.0) and not self._y_scrollbar.winfo_ismapped()
            ):  # y scrollbar needed
                self._hide_y_scrollbar = False
                self._create_grid_for_text_and_scrollbars(re_grid_y_scrollbar=True)
            elif (
                self._textbox.yview() == (0.0, 1.0) and self._y_scrollbar.winfo_ismapped()
    ...
    def image_create(self, index, **kwargs):
        raise AttributeError(
            "embedding images is forbidden, because would be incompatible with scaling"
        )
    ...
    def tag_config(self, tagName, **kwargs):
        if "font" in kwargs:
            raise AttributeError(
                "'font' option forbidden, because would be incompatible with scaling"
            )
    ...
    def window_cget(self, index, option):
        raise AttributeError(
            "embedding widgets is forbidden, would probably cause all kinds of problems ;)"
        )

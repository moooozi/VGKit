...
class CTkImage:
    ...
    def _check_images(self):
        ...
        if self._light_image is not None and not isinstance(self._light_image, Image.Image):
            raise ValueError(
                f"CTkImage: light_image must be instance if PIL.Image.Image, not {type(self._light_image)}"
            )
        if self._dark_image is not None and not isinstance(self._dark_image, Image.Image):
            raise ValueError(
                f"CTkImage: dark_image must be instance if PIL.Image.Image, not {type(self._dark_image)}"
            )

        # check values
        if self._light_image is None and self._dark_image is None:
            raise ValueError(
                "CTkImage: No image given, light_image is None and dark_image is None."
            )

        # check sizes
        if (
            self._light_image is not None
            and self._dark_image is not None
            and self._light_image.size != self._dark_image.size
        ):
            raise ValueError(
                f"CTkImage: light_image size {self._light_image.size} must be the same as dark_image size {self._dark_image.size}."
            )
    ...
    def create_scaled_photo_image(
        self, widget_scaling: float, appearance_mode: str
    ) -> "ImageTk.PhotoImage":

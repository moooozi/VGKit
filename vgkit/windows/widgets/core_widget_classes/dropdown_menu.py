...
class DropdownMenu(tkinter.Menu, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    def __init__(
        ...
        **kwargs,
        ...
        self._fg_color = (
            ThemeManager.theme["DropdownMenu"]["fg_color"]
            if fg_color is None
            else self._check_color_type(fg_color)
        )
        self._hover_color = (
            ThemeManager.theme["DropdownMenu"]["hover_color"]
            if hover_color is None
            else self._check_color_type(hover_color)
        )
        self._text_color = (
            ThemeManager.theme["DropdownMenu"]["text_color"]
    ...
    def _check_font_type(font: any):
        if isinstance(font, CTkFont):
            ...
            raise ValueError(
                f"Wrong font type {type(font)} for font '{font}'\n"
                + "For consistency, vgkit requires the font argument to be a tuple of len 2 or 3 or an instance of CTkFont.\n"
                + "\nUsage example:\n"

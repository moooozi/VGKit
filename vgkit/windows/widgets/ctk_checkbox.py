...
class CTkCheckBox(CTkBaseClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # color
        self._fg_color = (
            ThemeManager.theme["CTkCheckBox"]["fg_color"]
            if fg_color is None
        ...
        # shape
        self._corner_radius = (
            ThemeManager.theme["CTkCheckBox"]["corner_radius"]
            if corner_radius is None
            else corner_radius
        )
        self._border_width = (
            ThemeManager.theme["CTkCheckBox"]["border_width"]
    ...
    def _draw(self, no_color_updates=False):
        ...
        if no_color_updates is False or requires_recoloring_1 or requires_recoloring_2:
            ...
            if self._check_state is True:
                ...
                self._canvas.itemconfig(
                    "checkmark", fill=self._apply_appearance_mode(self._checkmark_color)
                )
            else:
                self._canvas.itemconfig(
                    "inner_parts",
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        if "variable" in kwargs:
            if self._variable is not None and self._variable != "":
                self._variable.trace_remove(
                    "write", self._variable_callback_name
                )  # remove old variable callback
